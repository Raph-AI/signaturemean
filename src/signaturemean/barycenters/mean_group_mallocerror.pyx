...
cpdef cnp.ndarray[double, ndim=1] mean(
    ...
    Parameters
    ----------
    datasig: numpy.ndarray
        Batch of signatures to average. Caution: before computing signatures,
        make sure to use :func:`signaturemean.utils.datascaling` on the paths
        dataset.
    ...
    pp. 169–229.
    """
    print('Step 1: initialization of variables.')
    ...
    for K in range(2, depth+1):
        ...
        for i in range(batch):
            ...
            q[i, left:right] = np.zeros(right-left)
            p[i, left:right] = np.zeros(right-left)
            for j in range(K-1):
                ...
                for depth1 in range(K-(j+1)):
                    print('K, i, j, depth1',K, i, j, depth1)
                    ...
                    # )
                print('update p')
                # print('len p_coeffs', len(p_coeffs))
                # print('len v...', len(v[j+1, i, left:right]))
                p[i, left:right] += p_coeffs[j]*v[j+1, i, left:right]
        print('update a')
        print('shape datasig', len(datasig), len(datasig[0]))
        print('shape q', len(q), len(q[0]))
        print('shape p', len(p), len(p[0]))
        print('1')
        print(len(a))
        print(dinds[K])
        print(a)
        ...
        # a[left:right] = -np.mean(datasig[:, left:right], axis=0)

        # a[left:right] = -np.mean(
        #     datasig[:, left:right] \
        #     + q[:, left:right] \
        #     + p[:, left:right],
        #     axis=0
